...
export function ActSelectorOverlay({
  isOpen,
  onClose,
  scenes,
  activeSceneId,
            ...
            <div className="pointer-events-auto mx-auto flex min-h-0 w-full max-w-7xl flex-1 flex-col overflow-y-auto">
              <header className="shrink-0">
                ...
                <p className="mt-3 max-w-2xl text-sm font-semibold uppercase tracking-[0.18em] text-white/50">
                  Choose the business scene to play on the landing screen.
                </p>
              </header>
                ...
                {scenes.map((scene, index) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`group min-h-[320px] border p-5 text-left transition ${
                        isActive
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className={`text-xs font-black uppercase tracking-[0.28em] ${isActive ? "text-orange-600" : "text-orange-400"}`}>
                            {scene.actLabel}
                          </p>
                          <h3 className="mt-3 text-2xl font-black uppercase leading-none tracking-tight sm:text-3xl">
                            {scene.sceneLabel}
                          </h3>
                        </div>
                        <ArrowRight className={`mt-1 h-5 w-5 transition group-hover:translate-x-1 ${isActive ? "text-black" : "text-white/50"}`} />
                      </div>

                      <p className={`mt-6 text-xs font-black uppercase tracking-[0.24em] ${isActive ? "text-black/55" : "text-white/45"}`}>
                        {scene.eyebrow}
                      </p>
                      <p className="mt-3 text-lg font-black uppercase leading-tight sm:text-xl">
                        {scene.overlayTitle}
                      </p>
                      <p className={`mt-4 text-xs font-semibold leading-5 sm:text-sm ${isActive ? "text-black/65" : "text-white/60"}`}>
